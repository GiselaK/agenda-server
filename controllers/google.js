...
exports.createEvent = function (accessToken, event, calID, next) {
  ...
  var googleReqEvent = {calendarId: calID, resource : {end:{dateTime: timeConverter.convertToRFC339(endDate)}, start:{dateTime: timeConverter.convertToRFC339(startDate)}}
  helpers.log("req:", googleReqEvent);
  request({method: 'POST', url: baseURL + '/calendars/' + calID + '/events', headers: {'Content-type': 'application/json', body: googleReqEvent}}, function (err, resp, body) {
    if (err) {
      var errStatusCode = JSON.parse(err).error.code;
